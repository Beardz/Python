...
def uploadOss(key,filename):
    ...
    oss2.resumable_upload(bucket, key, filename, progress_callback= percentage)
...
def upload():
    ...
    for file in files:
        ...
        data = res.read()
        print(time.strftime(r'%Y-%m-%d-%H-%M-%S', time.localtime())+'  '+version +' = '+ data.decode("utf-8"))
